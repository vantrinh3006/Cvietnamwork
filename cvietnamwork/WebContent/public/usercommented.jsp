			...
			<div class="body">
				...
				<%} %>
 				...
 				<div class="page">
					...
					%>
						<a href="<%=request.getContextPath()%>/public/user/commented?user_id=<%=userLogin.getId()%>&active=0&page=<%=1 %>"> « </a>
						<%	if(currentPage + 1 <= numberOfPages){ //chạy từ 1-> n-1
								for(int i = currentPage -1 ; i <= currentPage + 1 ; i ++ ){
									if(i==0){i=1;}	%>
									<a class="<%if(currentPage == i ){out.print("active");} %>"  href="<%=request.getContextPath()%>/public/user/commented?user_id=<%=userLogin.getId()%>&active=0&page=<%=i %>"> <%=i %> </a>
